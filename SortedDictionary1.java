...
public class SortedDictionary1 extends AbstractDictionary<Comparable, Object>{
	...
	int newIndexOf(Comparable key) {	
		...
		for(int i = 0; i < this.getSize() - 1; i++){
			...
			if(this.tabKeys[i] != null && this.tabKeys[i+1] == null){
				if(this.tabKeys[i].compareTo(key) <= 0){
					this.tabKeys[1] = this.tabKeys[0];
					this.tabVals[1] = this.tabVals[0];
					return 0;
				}
				else
					return i + 1;
